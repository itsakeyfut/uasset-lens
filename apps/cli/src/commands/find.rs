...
pub fn handle_find(
    ...
    let at = asset_type.map(|s| {
        serde_json::from_str::<uasset_lens_shared::AssetType>(&format!("\"{}\"", s))
            .unwrap_or_else(|_| uasset_lens_shared::AssetType::Unknown(s.to_string()))
    });

    let db = crate::open_db(db_path)?;

    let filter = uasset_lens_asset_db::AssetFilter {
        asset_type: at,
