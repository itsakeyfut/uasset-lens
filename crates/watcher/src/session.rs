...
impl WatchSession {
    ...
    pub fn init(&mut self) -> Result<(), WatchSessionError> {
        ...
        self.last_dead = dead_asset_detector::detect(&graph, &[])
            .into_iter()
            .collect();
    ...
    pub fn process_batch(
        ...
        if !to_scan.is_empty() {
            let result = scanner::scan_files(&to_scan, &self.content_root);
        ...
        let graph = build_graph(&self.db, &self.external_roots)?;
        let new_dead: HashSet<AssetPath> = dead_asset_detector::detect(&graph, &[])
            .into_iter()
            .collect();
