...
- [ ] `parser/properties.rs` — FProperty / FTag の解析（Blueprint のノード情報取得）
- [ ] Blueprint の Export オブジェクトから以下を抽出できるようにする
  - ノード総数（グラフノード数）
  - EventTick の使用有無
  - Cast ノードの数
  - 外部 Blueprint への参照数（Dependency Depth に相当）
- [ ] `AssetMetadata` に Blueprint 専用フィールドを追加、または専用構造体を用意
...
- [ ] Texture 重複検出（同一ファイルサイズ + 型 + 名前ベースの近似判定）
  - ファイルハッシュによる完全一致は将来対応（`xxhash-rust`）
- [ ] `DuplicateGroup` 構造体定義（重複 Asset のグループ）
- [ ] 単体テスト
...
- [ ] `LintViolation` 構造体（severity / rule_id / message / asset_path）
- [ ] Phase 4 で実装するルール
  - **命名規則**: プレフィックス検証（T_ / M_ / SM_ / BP_ / SK_ 等）
  - **Texture サイズ**: ファイルサイズ上限
  - **Blueprint 複雑度**: ノード数・EventTick 使用数
  - **LOD なし検知**: 将来の型情報取得後に対応

#### 設定ファイル拡張

- [ ] `.uasset-lens.toml` に `[lint]` セクションを追加（Phase 3 の `[scan]` に追記）
  ```toml
  [lint]
...
- [ ] テクスチャサンプル数の計測（Export プロパティから読み取り）
- [ ] MaterialInstance チェーン深度の計算（依存グラフから算出）
- [ ] `MaterialMetrics` 構造体定義
- [ ] 単体テスト
...
- [ ] `.uasset-lens.toml` の `[budget]` セクション定義
  ```toml
  [budget]
  Texture2D.max_size = 4194304    # 4 MB
  SoundWave.max_size = 2097152    # 2 MB
  ```
- [ ] `check_budget(assets, config) -> BudgetReport` 実装
  - カテゴリ別の超過 Asset 一覧
  - 超過数 / 超過率サマリー
...
- [ ] `blueprint <project_dir>` コマンド
  - 複雑度ランキング・警告一覧を表示
- [ ] `lint <project_dir>` コマンド
  - 全ルールを実行、違反一覧を表示
  - exit code `1` で CI ゲートとして使用可能
- [ ] `budget <project_dir>` コマンド
  - 予算超過 Asset 一覧・サマリー表示
- [ ] `duplicates <project_dir>` コマンド
  - 重複 Asset グループの表示
...
- [ ] `uasset-lens blueprint ./Project` が Blueprint の複雑度メトリクスを表示する
- [ ] `uasset-lens lint ./Project` が命名規則・複雑度違反を報告する
- [ ] `uasset-lens lint ./Project` が違反ありで exit code `1` を返す（CI ゲートとして機能）
- [ ] `.uasset-lens.toml` の `[lint]` 設定が反映される
- [ ] `uasset-lens budget ./Project` が予算超過 Asset を報告する
- [ ] `uasset-lens duplicates ./Project` が重複 Asset グループを検出する
- [ ] 全新コマンドで `--format json` が動作する

### テスト要件

- [ ] `cargo test --workspace` がパスする
- [ ] Blueprint フィクスチャで `bp-analyzer` のメトリクス抽出が検証されている
- [ ] `lint-engine` の各ルールが個別にテストされている
- [ ] `budget-tracker` の超過判定ロジックがテストされている

### 品質要件

- [ ] `cargo clippy --workspace -- -D warnings` が警告ゼロ
- [ ] `cargo fmt --check` がパスする
- [ ] `docs/specs/phases.md` Phase 4 の全機能が実装されている
