...
This release completes the CI-grade quality-gate workflow: the `check` command, the full `.uasset-lens.toml` schema, SARIF output, and the new `init` / `doctor` / `config validate` commands. Internally, the 15 library crates are consolidated into 8 by responsibility and the CLI is relocated to `apps/cli`.

### Added

#### apps/cli — check command
...
#### apps/cli — configuration (`.uasset-lens.toml`)
...
#### docs
- Reconcile `[lint.blueprint]` default thresholds between the spec and the code ([#346](https://github.com/itsakeyfut/uasset-lens/issues/346))

### Internal

- Consolidate the library crates from 15 to 8 by responsibility ([#334](https://github.com/itsakeyfut/uasset-lens/issues/334), [#335](https://github.com/itsakeyfut/uasset-lens/issues/335), [#336](https://github.com/itsakeyfut/uasset-lens/issues/336), [#337](https://github.com/itsakeyfut/uasset-lens/issues/337))
- Relocate the CLI to `apps/cli` (`crates/` = reusable libraries, `apps/` = applications) ([#342](https://github.com/itsakeyfut/uasset-lens/issues/342))
